...
if(strcmp($jsonapikey,DBAPIKEY)==0)
{
	switch( (string)$jsonaction )
	{
		...
		case "f_create":
		{
			...
			if(array_key_exists_r("creator|titl|desc|compid|comp|cntry|yrprod|type|genre|purl", $captureddata))
			{
				$the_creator_id = (int)$captureddata->creator;
				$the_title = (string)$captureddata->titl;
				$the_description = (string)$captureddata->desc;
				...
				if(!empty($objUser->data->query_result))
				{
					...
					if( AccessLevelValid((string)$jsonaction, $the_creator_access_level) )
					{
						...
						$json = $conn->doQuery("insert into `n_franchise` (`f_name`,`f_synopsis`,`f_company_id`,`f_company`,`f_country`,`f_year_production`,`f_type_id`,`f_genre_id_1`,`f_genre_id_2`,`f_genre_id_3`,`f_genre_id_4`,`f_genre_id_5`,`f_date_added`,`f_last_updated`,`f_creator_id`,`f_url_poster`) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?);",array($the_title,$the_description,$the_company_id,$the_company,$the_country,$the_year_production,$the_type,$the_genre_1, $the_genre_2, $the_genre_3, $the_genre_4, $the_genre_5, $conn->getDateTimeNow(),$conn->getDateTimeNow(),$the_creator_id,$the_poster_url),'json');
						$objUser = json_decode($json);
			...
			if(array_key_exists_r("creator|page", $captureddata))
			{
				...
				if(!empty($objUser->data->query_result))
				{
					...
					if( AccessLevelValid((string)$jsonaction, $the_creator_access_level) )
					{
						...
						$json = $conn->doQuery("select `f_id`,`f_name`,`f_company_id`,`f_company`,`f_country`,`f_year_production`,`f_synopsis`,`f_genre_id_1`,`f_genre_id_2`,`f_genre_id_3`,`f_genre_id_4`,`f_genre_id_5`,`f_type_id`,`f_url_poster`,`f_creator_id`,`f_is_active`,`f_date_added`,`f_last_updated` from `n_franchise` where `f_id`>=? ORDER BY `f_name` ASC LIMIT ? OFFSET ?;",array(1, $the_limit_per_page,$the_page_number),'json');
						$objUser = json_decode($json);
						...
						if( $the_keyword != "n/a" )
						{
							...
							$json = $conn->doQuery("select `f_id`,`f_name`,`f_company_id`,`f_company`,`f_country`,`f_year_production`,`f_synopsis`,`f_genre_id_1`,`f_genre_id_2`,`f_genre_id_3`,`f_genre_id_4`,`f_genre_id_5`,`f_type_id`,`f_url_poster`,`f_creator_id`,`f_is_active`,`f_date_added`,`f_last_updated` from `n_franchise` where `f_name` LIKE ? ORDER BY `f_name` ASC LIMIT ? OFFSET ?;",array("%$the_keyword%", $the_limit_per_page,$the_page_number),'json');
							$objUser = json_decode($json);
						}
		...
		case "f_edit":
		{
			...
			if(array_key_exists_r("creator|id|titl|desc|compid|comp|cntry|yrprod|type|purl|actv", $captureddata))
			{
				$the_creator_id = (int)$captureddata->creator;
				$the_id = (int)$captureddata->id;
				$the_title = (string)$captureddata->titl;
					...
					if( AccessLevelValid((string)$jsonaction, $the_creator_access_level) )
					{
						...
						{		
							if( $the_genre_is_passed_in == true )
							{
								...
								$json = $conn->doQuery("update `n_franchise` set `f_name`=?, `f_company_id`=?, `f_company`=?, `f_country`=?, `f_year_production`=?,`f_synopsis`=?, `f_genre_id_1`=?, `f_genre_id_2`=?, `f_genre_id_3`=?, `f_genre_id_4`=?, `f_genre_id_5`=?, `f_type_id`=?, `f_url_poster`=?, `f_is_active`=?, `f_last_updated`=? where `f_id`=?;",array($the_title,$the_company_id,$the_company,$the_country,$the_year_production,$the_description,$the_genre_1,$the_genre_2,$the_genre_3,$the_genre_4,$the_genre_5,$the_type,$the_poster_url,$the_is_active,$conn->getDateTimeNow(),$the_id),'json');
								$objUser = json_decode($json);
							}
							else
							{
								$json = $conn->doQuery("update `n_franchise` set `f_name`=?, `f_company_id`=?, `f_company`=?, `f_country`=?, `f_year_production`=?,`f_synopsis`=?, `f_type_id`=?, `f_url_poster`=?, `f_is_active`=?, `f_last_updated`=? where `f_id`=?;",array($the_title,$the_company_id,$the_company,$the_country,$the_year_production,$the_description,$the_type,$the_poster_url,$the_is_active,$conn->getDateTimeNow(),$the_id),'json');
								$objUser = json_decode($json);
							}
		...
		case "v_create":
		{
			...
			if(array_key_exists_r("creator|frid|frname|titl|sesn|epsd|yrprod|desc|drctrid|prdcrid|castsid|drctr|prdcr|casts|purl|ytid|cdn|prioyt|price|ss1|ss2|ss3|ss4|ss5", $captureddata))
			{
				$the_creator_id = (int)$captureddata->creator;
				...
				if(!empty($objUser->data->query_result))
				{
					...
					if( AccessLevelValid((string)$jsonaction, $the_creator_access_level) )
					{
						$json = $conn->doQuery("insert into `n_video` (`v_franchise_id`,`v_franchise_name`,`v_title`,`v_season`,`v_episode`,`v_year_production`,`v_synopsis`,`v_duration`,`v_director_id`,`v_producer_id`,`v_casts_id`,`v_director`,`v_producer`,`v_casts`,`v_url_poster`,`v_url_youtube_id`,`v_url_cdn`,`v_prioritize_youtube`,`v_price`,`v_screenshot_url_1`,`v_screenshot_url_2`,`v_screenshot_url_3`,`v_screenshot_url_4`,`v_screenshot_url_5`,`v_uploader_admin_id`,`v_updater_admin_id`,`v_date_uploaded`,`v_last_updated`) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?);",array($the_franchise_id,$the_franchise_name,$the_title,$the_season,$the_episode,$the_year_production,$the_description,$the_duration,$the_director_id,$the_producer_id,$the_casts_id,$the_director,$the_producer,$the_casts,$the_poster_url,$the_youtube_id,$the_cdn_url,$the_prioritize_youtube,$the_price,$the_screenshot_url_1,$the_screenshot_url_2,$the_screenshot_url_3,$the_screenshot_url_4,$the_screenshot_url_5,$the_creator_id,$the_creator_id,$conn->getDateTimeNow(),$conn->getDateTimeNow()),'json');
						$objUser = json_decode($json);
		...
		case "v_edit":
		{
			...
			if(array_key_exists_r("creator|id|frid|frname|titl|sesn|epsd|yrprod|desc|drctrid|prdcrid|castsid|drctr|prdcr|casts|purl|ytid|cdn|prioyt|price|ss1|ss2|ss3|ss4|ss5|actv", $captureddata))
			{
				$the_creator_id = (int)$captureddata->creator;
				$the_id = (int)$captureddata->id;
				$the_franchise_id = (string)$captureddata->frid;
				...
				if(!empty($objUser->data->query_result))
				{
					...
					if( AccessLevelValid((string)$jsonaction, $the_creator_access_level) )
					{
						...
						{		
							$json = $conn->doQuery("update `n_video` set `v_franchise_id`=?,`v_franchise_name`=?, `v_title`=?, `v_season`=?,`v_episode`=?, `v_year_production`=?, `v_synopsis`=?, `v_duration`=?, `v_director_id`=?, `v_producer_id`=?, `v_casts_id`=?,`v_director`=?, `v_producer`=?, `v_casts`=?, `v_url_poster`=?, `v_url_youtube_id`=?, `v_url_cdn`=?, `v_prioritize_youtube`=?, `v_price`=?, `v_screenshot_url_1`=?, `v_screenshot_url_2`=?, `v_screenshot_url_3`=?, `v_screenshot_url_4`=?, `v_screenshot_url_5`=?, `v_updater_admin_id`=?, `v_last_updated`=?, `v_is_active`=? where `v_id`=?;",array($the_franchise_id,$the_franchise_name,$the_title,$the_season,$the_episode,$the_year_production,$the_description,$the_duration,$the_director_id,$the_producer_id,$the_casts_id,$the_director,$the_producer,$the_casts,$the_poster_url,$the_youtube_id,$the_cdn_url,$the_prioritize_youtube,$the_price,$the_screenshot_url_1,$the_screenshot_url_2,$the_screenshot_url_3,$the_screenshot_url_4,$the_screenshot_url_5,$the_creator_id,$conn->getDateTimeNow(),$the_is_active,$the_id),'json');
							$objUser = json_decode($json);
